...
import { knex_db } from '../database.js';
import { google } from "googleapis";
import { ChartMainPosition, Track } from "../chart/models.js";
import { Performer } from "../artists/models.js";

let jsonParser = bodyParser.json()
export const apiRouter = express.Router();

apiRouter.post("/parse", jsonParser, async (req, res) => {
    let script = "(chart_date, track_position, track_id) VALUES "
...
apiRouter.get("/update", async (req, res) => {
    const result = await knex_db.raw(`select tr.title, tr.performer,
        count(p.id) as days,
...
apiRouter.get("/update/:year", async (req, res) => {
    const result = await knex_db.raw(`select tr.title, tr.performer,
        ...
        from track tr
        left join positions p on p.track_id = tr.id
        where tr.chart_year = '${req.params.year}'
        group by tr.id
